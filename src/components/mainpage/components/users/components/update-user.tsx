...
import { RegisterDataI } from "../../../../../app/slices/branded/auth/auth-types/auth.types";
...
export function FetchUserForUpdate() {
  ...
  return oneUser ? (
    <UpdateUser userDetails={oneUser} />
...
export function UpdateUser({ userDetails }: { userDetails: UserI }) {
  ...
  const [user, { isLoading, isError, isSuccess, error }] =
    useUpdateUserMutation();
  useEffect(() => {
    dispatch(
      setKeyValue({
  ...
  async function submit() {
    ...
    if (call) {
      ...
      if (data.firstName || data.lastName || data.email || data.password) {
        const d = deleteEmptyKeysAndValues<RegisterDataI>(data);

        await user({ ...d, id: userDetails!.id! });
