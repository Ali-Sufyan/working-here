...
import { isMyProperty } from "../../../checkkey-in-object";
import { regexCheckFormFields } from "../../../forms/hooks/check-fields-with-regex";
import { handleFormChange } from "../../../forms/hooks/handle-change";
import { validateFormKeys } from "../../../forms/hooks/validateFields";
import { Input } from "../../../forms/input";
import { SelectInput, SelectInput2, SelectInput3 } from "../../../forms/select";
import { myIcons } from "../../../icons";
import { ImportantIcon } from "../../../icons/util-icons";
...
import PresetQuillEditor from "../../../textbox/rich-text-ui";
import { convertDateFormat } from "../../../time-magic";
import { capitalize, findKeyByValue, mergeCssClass, parseNotification, RenderHtml, splitByUppercase } from "../../../utils";
import { CRUHeadingI, CruI } from "../interface.gen";
...
//            validateFormKeys(
//              { [field[d.key]]: selector[field[d.key]] },
//              dispatch
//            );
...
//     // we want to convert the array to an object
...
//     arrayfield.forEach((fi:(CRUHeadingI&{
//       data:any    })[]) => {
...
//     // If all required fields are present, update the state with the new nested array
//     setArrayfield(
//       arrayfield
...
updatedArray.forEach(
  ...
  // Prepare the updated value for dispatching
  // Update arrayValfield state similarly
  // const updatedValArray = arrayValfield.filter((_, index) => index !== indexToDelete);
  // setValArrayfield(updatedValArray);

  // Get the current value associated with hd.key
  // const valued = selector[field[hd.key]]?.value || [];

  
  // Create a copy of the valued array and remove the item at indexToDelete
  // const updatedValued = valued.filter((_, index) => index !== indexToDelete);
  
  // console.log({arrayValfield, valued, indexToDelete, hd, updatedValued});
  // // Dispatch the action with the updated valued array
  // dispatch(setKeyValue({ [field[hd.key]]: { value: updatedValued } }));
}
  return (
    <>
      {data.categories.map((category, k) => {
        return (
          <div
            key={k}
            className=" text-gray-900 rounded-lg  shadow-[1px_1px_0px_0px_hsla(280,70%,80%,0.3),-1px_-1px_0px_0px_hsla(280,70%,80%,0.1)] m-3  p-2"
          >
            <Typography
              sx={{
                display: "flex",
                                          ...
                                          if (
                                            ...
                                            if (filter) {
                                              ...
                                              const da = (
                                                ...
                                                acc[curr[ch.filter!.childKey]] =
                                                  curr[ch.filter!.childValue];
                                                return acc;
                                              }, {} as Record<string, any>);

                                              const linkd = data.headings.find(
                                                (h) => h.key === ch.filter!.link
                                              );
                                              if (linkd) {
                                                linkd.kv = da;
                                              }
                ...
                .map((hd) => (
                  <>
                    ...
                    {hd.formType === "array" &&
                      category.key === hd.category &&
                      hd.child && (
                        <div className="mt-2 text-gray-900 cursor-pointer flex flex-row align-middle place-items-center custom-elevated-paper px-1">
